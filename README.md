...
- 或手动放一篇进 `news/`：一个 `.mp3` 和一个**同名** `.txt`

```
news/2026-09-20/01_桜.mp3
...
音频走 HLS 分片下载，抓完会拿播放列表里 `#EXTINF` 之和验收时长并检查能否完整解码——
分片掉几个时 ffmpeg 常常照样返回成功，只看返回码查不出来。不完整会自动换一种下载方式重来。
...
- **合成音的调型不一定对**。实测 macOS `say` 读 10 个最小对有 3 个和词典不符。看台阶比听参考音可靠。
