...
5. macOS 上还可以在「自己写一句」里打一句日语，用系统朗读（Kyoko / Otoya）生成参考
...
- 「自己写一句」的系统朗读用的是 macOS 的 `say` 命令，**其它系统上这块会自动隐藏**。
  声音下拉框里的 **Otoya（男）在较新的 macOS 上可能没有预装**，这时 `say` 会不报错地退回到默认日语声音
  （也就是 Kyoko），听起来像是选项没生效。用 `say -v '?' | grep ja_JP` 看本机实际有哪些日语声音，
  要补装就去「系统设置 → 辅助功能 → 朗读内容 → 系统声音」。
