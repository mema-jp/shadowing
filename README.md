...
### AI 教学建议（可选，要 API key）

三个分数告诉你「差多少」，但「这次该改哪里」一直是规则做不好的：它挑当次偏差最大的
那一拍，而常有四五拍偏得一样多——实测把 25 遍随机分成两半，两半选出同一拍的比例只有
**14%**，所以每次说的都不一样，人无法收敛。

配了 key 之后，逐拍数据交给 Claude 出建议。**测量仍然归本地**（对齐、音高、时长、
调型），模型只做最后一步判断；它看不到音频，system 里写明了每个数字的含义和可靠边界。

```bash
pip install anthropic
export ANTHROPIC_API_KEY=sk-ant-...
python tutor.py            # 拿最近一次真实录音试跑一次
```

没配 key 或调用失败时自动退回规则版反馈，不影响其它功能。
单次约 900 token 输入，Opus 5 约 $0.018／次。
